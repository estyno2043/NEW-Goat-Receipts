...
class maisonmodal(ui.Modal, title="Maison Margiela"):
    ...
    price = ui.TextInput(label="Price without currency", placeholder="178.96", required=True)
    shipping = ui.TextInput(label="Shipping Costs", placeholder="10.00", required=True)
    tax = ui.TextInput(label="Tax Costs", placeholder="10.00", required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT name, street, city, zip, country FROM user_credentials WHERE user_id = ?", (str(owner_id),))
...
class maisonmodal2(ui.Modal, title="Maison Margiela Receipt"):
    currency = ui.TextInput(label="Currency ($, €, £)", placeholder="€", required=True, min_length=1, max_length=2)
    color = ui.TextInput(label="Color", placeholder="Black", required=True)
    size = ui.TextInput(label="Size", placeholder="M", required=True)
    orderdate = ui.TextInput(label="Order Date (DD/MM/YYYY)", placeholder="14/06/2024", required=True)
    deliverydate = ui.TextInput(label="Delivery Date (DD/MM/YYYY)", placeholder="17/04/2024", required=True)
...
# For backwards compatibility and proper imports
