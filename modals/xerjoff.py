...
class XerjoffStatusModal(ui.Modal, title="Xerjoff Receipt Type"):
    def __init__(self):
        super().__init__()

    async def on_submit(self, interaction: discord.Interaction):
        # This modal doesn't have inputs, it's handled by the view
        pass
...
class XerjoffPaymentModal(ui.Modal, title="Xerjoff Payment Confirmation"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            view = EmailChoiceView(owner_id, receipt_html, receipt_filename, "customer@xerjoff.com", "Xerjoff Customer Service", f"Payment Confirmation - {self.referencenum.value}")
...
class XerjoffOrderModal(ui.Modal, title="Xerjoff Order - Step 1/2"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        xerjoff_form_data[str(owner_id)] = {
...
class XerjoffOrderSecondModal(ui.Modal, title="Xerjoff Order - Step 2/2"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if 'xerjoff_form_data' not in globals() or str(owner_id) not in xerjoff_form_data:
            await interaction.response.send_message("Session expired. Please start over.", ephemeral=True)
            return
        
        first_data = xerjoff_form_data[str(owner_id)]
        ...
        try:
            ...
            from emails.choise import EmailChoiceView
            view = EmailChoiceView(owner_id, receipt_html, receipt_filename, "customer@xerjoff.com", "Xerjoff Customer Service", f"Order Confirmation - {self.referencenum.value}")
            
            await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
            
            # Clean up stored data
            del xerjoff_form_data[str(owner_id)]
...
def get_xerjoff_view(owner_id):
    ...
    return embed, XerjoffStatusView(owner_id)
