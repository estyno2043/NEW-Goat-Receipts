...
def is_adwysd_link(link):
    adwysd_pattern = re.compile(r'^https?://(www\.)?(adwysd\.net|uk\.alwaysdowhatyoushoulddo\.com)/.*')
    return bool(adwysd_pattern.match(link))

class adwysdmodal(ui.Modal, title="discord.gg/goatreceipt"):
    Link = discord.ui.TextInput(label="Product link *", placeholder="https://adwysd.net/...", required=True)
    ...
    arrivaldate = discord.ui.TextInput(label="Arrival date *", placeholder="6/5/2025", required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_details:
            ...
            arrivaldate = self.arrivaldate.value

            if not is_adwysd_link(Link):
                embed = discord.Embed(title="Error - Invalid ADWYSD link", description="Please provide a valid ADWYSD link.")
            ...
            from addons.nextsteps import NextstepAdwysd
            embed = discord.Embed(title="Next Page", description="Click 'Next Page' to continue to the next set of inputs.")
            await interaction.response.send_message(content=f"{interaction.user.mention}", embed=embed, view=NextstepAdwysd(owner_id), ephemeral=False)
        else:
            # Handle case where no user details are found
            embed = discord.Embed(title="Error", description="No user details found. Please ensure your information is set up.")
            await interaction.response.send_message(embed=embed, ephemeral=True)

class adwysdmodal2(ui.Modal, title="ADWYSD Receipt"):
    size = discord.ui.TextInput(label="Product size *", placeholder="M", required=True)
