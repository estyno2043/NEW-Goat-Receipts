...
class diormodal(ui.Modal, title="Dior Receipt"):
    ...
    price_info = discord.ui.TextInput(
        label="Price (format: subtotal,shipping,tax)", 
        placeholder="100.00,10.00,10.00", 
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_details:
            ...
            price_parts = self.price_info.value.split(',')
            if len(price_parts) != 3:
                embed = discord.Embed(title="Error - Invalid price format", description="Please provide prices in format: subtotal,shipping,tax")
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            
            try:
                subtotal = float(price_parts[0].strip())
                shipping = float(price_parts[1].strip())
                tax = float(price_parts[2].strip())
            except ValueError:
                embed = discord.Embed(title="Error - Invalid price values", description="Please provide valid numeric values for prices")
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if match:
                        ...
                        if view_item_data:
                            ...
                            embed = discord.Embed(title="Error", description="Could not find product price information on the page. Please try another product.")
                ...
                fulltotal = subtotal + shipping + tax
                ...
                html_content = html_content.replace("{price}", str(subtotal))
