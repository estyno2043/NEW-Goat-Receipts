...
async def on_ready():
    ...
    # Sync commands after all registrations
    try:
...
class RedeemKeyModal(ui.Modal, title="Redeem License Key"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if result["success"]:
            ...
            # Try to add customer role and subscription roles to the user
            try:
                with open("config.json", "r") as f:
                    import json
                    config = json.load(f)
                ...
                if client_role_id > 0:
                    ...
                    if guild:
                        ...
                        # Add customer role to everyone who redeems a key
                        customer_role = discord.utils.get(guild.roles, id=1412498223842721903)
                        if customer_role:
                            await self.interaction.user.add_roles(customer_role)
                        ...
                        if "1month" in subscription_type or "1month_product" in subscription_type or "3month" in subscription_type or "30day" in subscription_type or "guild_30days" in subscription_type or "guild_lifetime" in subscription_type or "lifetime" in subscription_type.lower():
                            ...
                            if subscription_role:
                                await self.interaction.user.add_roles(subscription_role)
                                print(f"Added subscription role to {self.interaction.user.display_name}")
            except Exception as e:
...
# File upload commands will be registered in on_ready event
